...
const getPassive = (name: string, rarity: string, originalPassive: any) => {
    let value = 1;
    let cap = 10;
    let description = '';
    ...
    if (originalPassive?.name.includes('Cosmic')) {
       // Keep original vibe
       return {
    ...
    return {
        name: originalPassive?.name || 'Loyal Companion',
        description,
        effectType: 'bonus_gold',
        value,
        capPerDay: cap
...
const petDB = data.pets.map((p: any) => {
    // skip duplicates if necessary or just write them out. Cat is duplicated in export?
    return `    '${p.id}': {
        id: '${p.id}', name: '${p.name}', icon: '${iconMap[p.id] || '🐾'}',
